...
export function linearInterpolate(
  ...
  console.assert(
    from.x < to.x,
    "linearInterpolate(): to.x must be greater than from.x"
  );
  console.assert(
    from.x <= marker,
    "linearInterpolate(): from.x must be less than or equal to p"
  );
  console.assert(
    marker <= to.x,
    "linearInterpolate(): p must be less than or equal to to.x"
  );

  return from.y + ((marker - from.x) * (to.y - from.y)) / (to.x - from.x);
}

export function linearInterpolatePoints(
  ...
  console.assert(
    positions.length === steps,
    "positions.length and steps aren't equal"
  );
  return positions;
}

export function choices(map: { [key: string]: any }): string | null {
  let cumSum = 0;
  let cumWeights: { [key: string]: any } = {};
